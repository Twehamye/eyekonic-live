...
from .models import Member,Relative,ChildrenDetail,Deposit,Expense,Asset,Fine,FinePayment
from .models import *
from django.contrib.auth import authenticate, login, logout
from .forms import MemberForm,RelativeForm,DepositForm,ChildrenDetailForm,CreateUserForm,FineForm,FinePaymentForm,AssetForm,OtherIncomeForm
...
def report(request):
    ...
    cursor.execute('select sum(amount) from eyekonic_app_fine union select sum(amount) from eyekonic_app_deposit')
    f = cursor.fetchone()
...
def allFines(request):
    ...
    if search_fine:
        ...
        fines_summary=FinePayment.objects.annotate(total_fines=Sum('amount'))
